...
public class CustomerController {
    ...
    public ResponseEntity<?> signIn(@Valid @RequestBody LoginRequest request) {
        try {
            ...
            log.info("Customer logged successfully");
            return ResponseEntity.ok(jwtResponse);
        } catch (AuthenticationException e) {
            log.error("Password or/and email is wrong");
    ...
    public ResponseEntity<String> signUp(@Valid @RequestBody SignUpRequest request) {
        try {
            ...
            log.error("Customer with given email already exists");
    ...
    public ResponseEntity<?> getCustomerOrders(@RequestParam String email, @RequestParam(defaultValue = "0") int page) {
        try {
            ...
            log.error("Customer with given email does not exist");
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
