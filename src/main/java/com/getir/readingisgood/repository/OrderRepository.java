...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    Page<Order> findAllByCustomerEmail(@NotBlank String email, Pageable pageable);

    @Query("select new com.getir.readingisgood.model.StatisticsDto(to_char(o.orderDateTime, 'Month') as month, count(o.customer.email) as orderCount) " +
            "from Order o " +
            "where o.customer.email = ?1 " +
            "group by month")
    List<StatisticsDto> getOrdersCountByMonth(String email);
